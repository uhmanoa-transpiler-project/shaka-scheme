...
auto print_lexstate = [](LexerInput& input) {
...
auto prefix_message = [](std::string message) {
  return [=](LexerInput& input) {
    std::cout << message << std::endl;
  };
};

auto replace_with = [](const std::string s) {
  return [=](LexResult result) {
    if (result.is_token()) {
      result.str = s;
    }
...
std::function<LexResult(LexResult)> replace_hex_escape = [](LexResult result) {
  if (result.is_token()) {
...
std::function<LexResult(LexResult)> replace_hex_scalar_value = [](LexResult
                                                                  result) {
  if (result.is_token()) {
...
std::function<LexResult(LexResult)>
    replace_mnemonic_escape = [](LexResult result) {
  if (result.is_token()) {
    if (result.str == "\\a") {
      result.str = "\a";
